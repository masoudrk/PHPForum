angular.module(appName).controller('AnswersCtrl', function ($scope, $rootScope, $routeParams, $state, $location, $timeout, Extention, $stateParams, $uibModal) {
    ...
    $scope.changeAnswerState = function (uid, s) {
        Extention.post('changeAnswerAccepted', {
            State: s,
            Score: uid.rate,
            AnswerID: uid.ID,
            AdminPermissionLevel: session.AdminPermissionLevel,
            UserID: uid.AuthorID,
            AuthorID: uid.QuestionAuthorID,
            QuestionID: uid.QuestionID
        }).then(function (res) {
        ...
        var modalInstance = $uibModal.open({
            ...
            controller: function ($scope, $uibModalInstance) {
                ...
                $scope.openDiscardModal = function () {
                    $uibModalInstance.close({message: 'openDiscardModal', data: $scope.Answer});
                };
                $scope.openScoreModal = function () {
                    $uibModalInstance.close({message: 'openScoreModal', data: $scope.Answer});
                };
                $scope.editAnswer = function () {
                    if ($scope.AnswerText) {
                        Extention.post('editAnswer', {
    ...
    $scope.openAttachmentsModal = function (AnswerID) {
        Extention.post('Attachments/getAllAttachments', {AnswerID: AnswerID}).then(function (res) {
            if (res && res.Status === 'success') {
                $uibModal.open({
                    animation: true,
