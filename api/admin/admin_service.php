...
$app->post('/getAllUsers', function() use ($app)  {
   // adminRequire();
    ...
    if(isset($data->searchValue) && strlen($data->searchValue) > 0){
        ...
        $where = "WHERE (Username LIKE '%".$s."%' OR FullName LIKE '%".$s."%' OR Email LIKE '%".$s."%')";
        $hasWhere = TRUE;
    }

    $pageRes = $pr->getPage($db,"SELECT user.*,  admin.ID as 
AdminID FROM user LEFT JOIN admin on admin.UserID = user.ID  ".$where." ORDER BY user.ID ASC");
